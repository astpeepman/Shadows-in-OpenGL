...
public:
    ...
    Model(string const& path, vec3 pos, bool gamma = false) : gammaCorrection(gamma)
    {
        model = mat4(1.0f);
        position = pos;
    ...
    virtual void Draw(const Shader& s, bool IZO)
    {
    ...
    void setproj(mat4 prj, Shader s) {
        s.setMat4("projection", prj);
    }
    void setview(mat4 view, Shader s) {
        s.setMat4("view", view);
    }
